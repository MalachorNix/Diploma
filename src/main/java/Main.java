...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        mode(width, height, n, m, xyRange, gauss);
    ...
    private static void mode(int width, int height, int n, int m, double xyRange, double gauss) {
        Complex[][] result = new Complex[width][height];
        double stepX = 2 * xyRange / width;
        double stepY = 2 * xyRange / height;

        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                result[i][j] = new Complex(HermiteGaussianModes.hermiteGauss2D(n, m, -xyRange + i * stepX, -xyRange + j * stepY, gauss));
            }
        }
    ...
    private static void transform2D(int width, int height, int n, int m, double z, double k, double yMin, double xMin, double stepY, double stepX, double gauss, double u, double v, Complex[][] function, double stepU, double stepV) {
        ...
        Graph.draw2DIntensity(function, "pictures/intensityOutput.bmp");
        Graph.draw2DPhase(function, "pictures/phaseOutput.bmp");
    }
